...
const SortOptions = ({ sortValue, setSortValue }: Props) => {
  return (
    ...
    >
      <p className="mb-2 text-sm font-semibold">Sort Results By</p>

      <Listbox.Button className="flex items-center justify-between w-full px-4 py-2 text-sm text-left rounded shadow bg-background">
      ...
      <Listbox.Options className="absolute w-full px-1 py-2 mt-1 text-sm border rounded shadow border-primary bg-background top-16 focus:outline-none">
        {sortOptions.map((item) => (
          <Listbox.Option
            ...
            value={item}
            className={({ active }) =>
              `cursor-pointer rounded  ${active ? "bg-primary" : ""}`
            }
          >
            {({ selected }) => (
              <>
                <span
                  className={`block rounded py-2 pl-4  ${
